...
class Linear(nn.Linear):
    ...
    def __init__(self, in_features: int, out_features: int, init_scale=1, init_bias=0.0, bias: bool = True, device=None,
                 dtype=None):
...
class ActorLSTM(Actor):
    ...
    @jit.script_method
    def forward(self, obs: Tensor, states: Tensor, masks: Tensor, deterministic: bool) -> Tuple[
...
class MlpPolicyValue(PolicyValue):
    ...
    @jit.script_method
    def compute_loss(self, obs, returns, actions, old_values, old_neglogpacs, advs, cliprange, ent_coef: float,
                     vf_coef: float):
        obs = self._normalize_obs(obs)
...
class LSTMPolicy(PolicyValue):
    ...
    @jit.script_method
    def compute_loss(self, obs, returns, masks, actions, old_values, old_neglogpacs, advs, states, cliprange,
                     ent_coef: float, vf_coef: float):
        obs = self._normalize_obs(obs)
        states = torch.permute(states, (1, 0, 2))
